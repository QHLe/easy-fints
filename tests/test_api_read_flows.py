...
def test_accounts_requires_tan_then_confirm(fake_backend):
    status_code, payload = unwrap_response(api.accounts({"config": {"scenario": "accounts_tan"}}))
    ...
    confirm_status, accounts = unwrap_response(api.confirm({"session_id": session_id, "tan": "123456"}))
...
def test_transactions_decoupled_confirmation_loop(fake_backend):
    status_code, payload = unwrap_response(
        api.transactions({"config": {"scenario": "transactions_decoupled"}, "days": 5})
    )
    ...
    session_id = payload["session_id"]
    first_status, first_payload = unwrap_response(api.confirm({"session_id": session_id}))
    assert first_status == 202
    assert first_payload["state"] == "awaiting_decoupled"

    second_status, transactions = unwrap_response(api.confirm({"session_id": session_id}))
